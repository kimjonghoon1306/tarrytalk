// [임시] 라이브 RTDB 규칙 messages type에 voice 추가. 사용 후 삭제.
...
module.exports = async (req, res) => {
  try {
    if ((req.query.key || '') !== 'voice-2026-x7k9') return res.status(403).json({ error: 'forbidden' });
    ...
    if (txt.includes('|voice)')) return res.json({ ok: true, already: true });
    const before = txt;
    txt = txt.replace(/system\|sticker\|poll\)/g, 'system|sticker|poll|voice)');
    if (txt === before) return res.status(500).json({ error: 'pattern_not_found', sample: before.slice(0, 3000) });
